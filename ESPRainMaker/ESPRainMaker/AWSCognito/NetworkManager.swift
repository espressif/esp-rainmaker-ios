...
class NetworkManager {
    ...
    var bleProxyReportWorkItems: [String: DispatchWorkItem] = [:]
    let bleProxyReportDelay: TimeInterval = 0.5
    ...
    func getNodeInfo(nodeId: String, completionHandler: @escaping (Node?, ESPNetworkError?) -> Void) {
        if Configuration.shared.appConfiguration.supportLocalControl,
           let availableService = User.shared.localServices[nodeId],
        ...
        } else if User.shared.getNode(id: nodeId)?.isCloudParamTransportAvailable() == true {
    ...
    func getNodeInfoPrivate(nodeId: String, completionHandler: @escaping (Node?, ESPNetworkError?) -> Void) {
        if ESPNetworkMonitor.shared.isConnectedToNetwork {
    ...
    func getDeviceParam(device: Device, completionHandler: @escaping (ESPNetworkError?) -> Void) {
        ...
        if Configuration.shared.appConfiguration.supportLocalControl {
            if let nodeid = device.node?.node_id {
                ...
                } else if device.node?.isCloudParamTransportAvailable() == true {
    ...
    func getDeviceParamPrivate(device: Device, completionHandler: @escaping (ESPNetworkError?) -> Void) {
        if ESPNetworkMonitor.shared.isConnectedToNetwork {
    ...
    func setDeviceParamPrivate(nodeID: String?, parameter: [String: Any], completionHandler: @escaping (ESPCloudResponseStatus) -> Void) {
        if ESPNetworkMonitor.shared.isConnectedToNetwork {
