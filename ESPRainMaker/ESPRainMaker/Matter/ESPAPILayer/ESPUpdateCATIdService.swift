...
class ESPUpdateCATIdService {
    ...
    func initMatterController(groupId: String, group: ESPNodeGroup) {
        ...
        if let noc = self.fabricDetails.getUserNOCDetails(groupId: groupId) {
            commissioner.initializeMTRControllerWithUserNOC(matterFabricData: group, userNOCData: noc)
        }
    }
        ...
        if let matterNodes = self.matterNodes, index < matterNodes.count {
            ...
            if let nodeId = node.node_id, let groupId = self.fabricDetails.getGroupId(nodeId: nodeId), let group = self.fabricDetails.getGroupData(groupId: groupId), let matterNodeId = node.matter_node_id, User.shared.isMatterNodeConnected(matterNodeId: matterNodeId), let deviceId = matterNodeId.hexToDecimal {
                ...
                if let _ = commissioner.sController {
                    if let group = commissioner.group, let grpId = group.groupID, grpId != groupId {
                        self.initMatterController(groupId: groupId, group: group)
                    }
                } else {
                    self.initMatterController(groupId: groupId, group: group)
                }
                commissioner.readAllACLAttributes(deviceId: deviceId) { entries in
                    if let entries = entries, let fabricDetails = group.fabricDetails, let catIdOperate = fabricDetails.catIdOperateDecimal {
                        ...
                        if newEntries.0.count > 0, newEntries.1 == true {
                            commissioner.writeAllACLAttributes(deviceId: deviceId, accessControlEntry: newEntries.0) { status in
