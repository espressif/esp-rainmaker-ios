    ...
    func getTempMeasurementCluster(groupId: String, deviceId: UInt64, completion: @escaping (MTRBaseClusterTemperatureMeasurement?) -> Void) {
        ...
        if let controller = sController, endpointClusterId.0 == true, let key = endpointClusterId.1, let endpoint = UInt16(key) {
            if let device = try? controller.getDeviceBeingCommissioned(deviceId) {
                if let cluster = MTRBaseClusterTemperatureMeasurement(device: device, endpoint: endpoint, queue: self.matterQueue) {
                    completion(cluster)
                } else {
                    completion(nil)
                }
                ...
                controller.getBaseDevice(deviceId, queue: self.matterQueue) { device, _ in
                    if let device = device, let cluster = MTRBaseClusterTemperatureMeasurement(device: device, endpoint: endpoint, queue: self.matterQueue) {
                        completion(cluster)
                    } else {
                        completion(nil)
                    }
