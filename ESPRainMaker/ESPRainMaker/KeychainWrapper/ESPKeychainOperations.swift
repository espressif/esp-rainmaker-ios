...
//
//  ESPKeychainOperations.swift
//  ESPRainMaker
//

import Foundation

class ESPKeychainOperations: NSObject {
    
    static let shared: ESPKeychainOperations = ESPKeychainOperations()
    
    /// Name of service
    var service: String {
        if let info = Bundle.main.infoDictionary, let bundleId = info["CFBundleIdentifier"] as? String {
            return "\(bundleId).cognitoUserData"
        }
        return "com.app.bundleId.cognitoUserData"
    }
    ...
    func add(value: Data, account: String) throws {
        ...
        guard status == errSecSuccess else { throw ESPKeychainErrors.setOperationError }
    }
    /**
    ...
    func delete(account: String) throws {
        ...
        guard status == errSecSuccess else { throw ESPKeychainErrors.deleteOperationError }
    }
    /**
    ...
    func exists(account: String) throws -> Bool {
        ...
        case errSecItemNotFound:
            return false
        default:
            throw ESPKeychainErrors.creatingError
        }
    }
}
