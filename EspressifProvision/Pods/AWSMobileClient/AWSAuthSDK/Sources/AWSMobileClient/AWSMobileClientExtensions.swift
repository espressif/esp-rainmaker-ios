...
public struct SessionToken {
    ...
    public var claims: [String: AnyObject]? {
        if tokenString == nil {
            ...
            let claimsData = Data(base64Encoded: updatedClaims, options: .ignoreUnknownCharacters)
...
extension AWSMobileClient: AWSIdentityProviderManager {
    ...
    public func logins() -> AWSTask<NSDictionary> {
        ...
        if federationProvider == .none {
            ...
            let userPoolsTokenTask: AWSTaskCompletionSource<NSDictionary> = AWSTaskCompletionSource()
            ...
            dict.addEntries(from: cachedLoginsMap)
        } else if federationProvider == .hostedUI {
            if !federationDisabled {
                let hostedUITokenTask: AWSTaskCompletionSource<NSDictionary> = AWSTaskCompletionSource()
                getTokens { tokens, error in
                    if let tokens = tokens {
                        dict[self.userPoolClient!.identityProviderName] = tokens.idToken!.tokenString!
        ...
        let task = AWSTask(result: dict as NSDictionary)
        return task
    }
}
...
extension AWSMobileClient {
    ...
    internal var userPoolClient: AWSCognitoIdentityUserPool? {
        return userpoolOpsHelper.userpoolClient
    }
    ...
    public func signIn(username: String,
        ...
        let user = userPoolClient?.getUser(username)
        ...
        if validationData != nil {
            validationAttributes = validationData!.map { AWSCognitoIdentityUserAttributeType(name: $0, value: $1) }
        }
        if userpoolOpsHelper.passwordAuthTaskCompletionSource != nil {
            ...
            userpoolOpsHelper.passwordAuthTaskCompletionSource!.set(result: authDetails)
            userpoolOpsHelper.passwordAuthTaskCompletionSource = nil
            ...
            let isCustomAuth = userPoolClient?.isCustomAuth ?? false
            if isCustomAuth {
    ...
    public func signUp(username: String,
        ...
        if userPoolClient == nil { completionHandler(nil, AWSMobileClientError.userPoolNotConfigured(message: "Cognito User Pools is not configured in `awsconfiguration.json`. Please add Cognito User Pools before using this API.")) }

        let userAttributesTransformed = userAttributes.map { AWSCognitoIdentityUserAttributeType(name: $0, value: $1) }
        let validationDataTransformed = validationData.map { AWSCognitoIdentityUserAttributeType(name: $0, value: $1) }

        userPoolClient?.signUp(username, password: password,
    ...
    public func confirmSignUp(username: String, confirmationCode: String, completionHandler: @escaping ((SignUpResult?, Error?) -> Void)) {
        if let uname = self.userpoolOpsHelper.signUpUser?.username, uname == username {
            confirmSignUp(user: userpoolOpsHelper.signUpUser!, confirmationCode: confirmationCode, completionHandler: completionHandler)
        } else {
            let user = userPoolClient?.getUser(username)
    ...
    public func resendSignUpCode(username: String, completionHandler: @escaping ((SignUpResult?, Error?) -> Void)) {
        if let uname = self.userpoolOpsHelper.signUpUser?.username, uname == username {
            resendSignUpCode(user: userpoolOpsHelper.signUpUser!, completionHandler: completionHandler)
        } else {
            let user = userPoolClient?.getUser(username)
    ...
    public func signOut(options: SignOutOptions = SignOutOptions(), completionHandler: @escaping ((Error?) -> Void)) {
        ...
        if federationProvider == .hostedUI {
            if options.invalidateTokens {
                AWSCognitoAuth(forKey: CognitoAuthRegistrationKey).signOut { error in
        ...
        if federationProvider == .userPools, options.signOutGlobally == true {
            _ = userpoolOpsHelper.currentActiveUser!.globalSignOut().continueWith { (task) -> Any? in
                if task.result != nil {
    ...
    public func signOut() {
        credentialsFetchCancellationSource.cancel()
        if federationProvider == .hostedUI {
            AWSCognitoAuth(forKey: CognitoAuthRegistrationKey).signOutLocallyAndClearLastKnownUser()
        }
        cachedLoginsMap = [:]
        customRoleArnInternal = nil
        setCustomRoleArnInternal(nil, for: self)
        saveLoginsMapInKeychain()
        setLoginProviderMetadataAndSaveInKeychain(provider: .none)
        performUserPoolSignOut()
        internalCredentialsProvider?.identityProvider.identityId = nil
        internalCredentialsProvider?.clearKeychain()
        mobileClientStatusChanged(userState: .signedOut, additionalInfo: [:])
        federationProvider = .none
        credentialsFetchCancellationSource = AWSCancellationTokenSource()
    }
    ...
    internal func performUserPoolSignOut() {
        if let task = self.userpoolOpsHelper.passwordAuthTaskCompletionSource?.task, !task.isCompleted {
            userpoolOpsHelper.passwordAuthTaskCompletionSource?.set(error: AWSMobileClientError.unableToSignIn(message: "Could not get end user to sign in."))
        }
        userpoolOpsHelper.passwordAuthTaskCompletionSource = nil
        invokeSignInCallback(signResult: nil, error: AWSMobileClientError.unableToSignIn(message: "Could not get end user to sign in."))
        userPoolClient?.clearAll()
    }
    ...
    internal func performUserPoolSuccessfulSignInTasks(session: AWSCognitoIdentityUserSession) {
        ...
        cachedLoginsMap = [self.userPoolClient!.identityProviderName: tokenString]
    ...
    internal func performHostedUISuccessfulSignInTasks(disableFederation: Bool = false,
                                                       session _: AWSCognitoAuthUserSession,
        ...
        if federationProviderIdentifier == nil {
            cachedLoginsMap = [self.userPoolClient!.identityProviderName: federationToken]
        } else {
            cachedLoginsMap = [federationProviderIdentifier!: federationToken]
        }
    ...
    internal func performFederatedSignInTasks(provider: String, token: String) {
        cachedLoginsMap = [provider: token]
        federationProvider = .oidcFederation
    ...
    internal func postSignInKeychainAndCredentialsUpdate(provider: FederationProvider, additionalInfo _: [String: String]) {
        saveLoginsMapInKeychain()
        setLoginProviderMetadataAndSaveInKeychain(provider: provider)
        internalCredentialsProvider?.clearCredentials()
        internalCredentialsProvider?.credentials(withCancellationToken: credentialsFetchCancellationSource)
    }
    ...
    public func confirmSignIn(challengeResponse: String,
        ...
        if userpoolOpsHelper.mfaCodeCompletionSource != nil {
            ...
            userpoolOpsHelper.mfaCodeCompletionSource?.set(result: challengeResponse as NSString)
        } else if userpoolOpsHelper.newPasswordRequiredTaskCompletionSource != nil {
            userpoolOpsHelper.currentConfirmSignInHandlerCallback = completionHandler
            let passwordDetails = AWSCognitoIdentityNewPasswordRequiredDetails(proposedPassword: challengeResponse,
                                                                               userAttributes: userAttributes)
            userpoolOpsHelper.newPasswordRequiredTaskCompletionSource?.set(result: passwordDetails)
        } else if userpoolOpsHelper.customAuthChallengeTaskCompletionSource != nil {
            userpoolOpsHelper.currentConfirmSignInHandlerCallback = completionHandler
            let customAuthDetails = AWSCognitoIdentityCustomChallengeDetails(challengeResponses: ["ANSWER": challengeResponse])
    ...
    public func changePassword(currentPassword: String, proposedPassword: String, completionHandler: @escaping ((Error?) -> Void)) {
        userpoolOpsHelper.currentActiveUser!.changePassword(currentPassword, proposedPassword: proposedPassword).continueWith { (task) -> Any? in
            if let error = task.error {
                completionHandler(AWSMobileClientError.makeMobileClientError(from: error))
            } else if let _ = task.result {
                completionHandler(nil)
    ...
    public func getAWSCredentials(_ completionHandler: @escaping (AWSCredentials?, Error?) -> Void) {
        if internalCredentialsProvider == nil {
        ...
        let cancellationToken = credentialsFetchCancellationSource
    ...
    public func releaseSignInWait() {
        if federationProvider == .userPools {
            userpoolOpsHelper.passwordAuthTaskCompletionSource?.set(error: AWSMobileClientError.unableToSignIn(message: "Unable to get valid sign in session from the end user."))
            userpoolOpsHelper.passwordAuthTaskCompletionSource = nil
            tokenFetchLock.leave()
        } else if federationProvider == .hostedUI {
            pendingGetTokensCompletion?(nil, AWSMobileClientError.unableToSignIn(message: "Could not get valid token from the user."))
            pendingGetTokensCompletion = nil
            tokenFetchLock.leave()
        } else if federationProvider == .oidcFederation {
            pendingAWSCredentialsCompletion?(nil, AWSMobileClientError.unableToSignIn(message: "Could not get valid federation token from the user."))
            pendingAWSCredentialsCompletion = nil
            credentialsFetchLock.leave()
        }
    }
...
extension AWSMobileClient {
    ...
    public func verifyUserAttribute(attributeName: String,
        ...
        let userDetails = AWSMobileClientUserDetails(with: userpoolOpsHelper.currentActiveUser!)
    ...
    public func confirmUpdateUserAttributes(attributeName: String, code: String, completionHandler: @escaping ((Error?) -> Void)) {
        confirmVerifyUserAttribute(attributeName: attributeName, code: code, completionHandler: completionHandler)
    }
...
extension AWSMobileClient: UserPoolAuthHelperlCallbacks {
    func getPasswordDetails(_: AWSCognitoIdentityPasswordAuthenticationInput,
        ...
        if userPoolClient?.isCustomAuth ?? false {
            let authDetails = AWSCognitoIdentityPasswordAuthenticationDetails(username: currentUser?.username ?? "",
        ...
        case .signedIn, .signedOutUserPoolsTokenInvalid:
            userpoolOpsHelper.passwordAuthTaskCompletionSource = passwordAuthenticationCompletionSource
            mobileClientStatusChanged(userState: .signedOutUserPoolsTokenInvalid, additionalInfo: ["username": self.userPoolClient?.currentUser()?.username ?? ""])
        default:
            break
        }
    }
    ...
    func getNewPasswordDetails(_: AWSCognitoIdentityNewPasswordRequiredInput,
    ...
    func getCustomAuthenticationDetails(_: AWSCognitoIdentityCustomAuthenticationInput,
        ...
        if currentUserState == .signedIn {
            let username = userPoolClient?.currentUser()?.username ?? ""
            mobileClientStatusChanged(userState: .signedOutUserPoolsTokenInvalid,
    ...
    func getCode(_ authenticationInput: AWSCognitoIdentityMultifactorAuthenticationInput, mfaCodeCompletionSource: AWSTaskCompletionSource<NSString>) {
        userpoolOpsHelper.mfaCodeCompletionSource = mfaCodeCompletionSource
        var codeDeliveryDetails: UserCodeDeliveryDetails?
...
extension AWSMobileClient {
    internal func setLoginProviderMetadataAndSaveInKeychain(provider: FederationProvider) {
        ...
        keychain.setString(provider.rawValue, forKey: FederationProviderKey)
        if let customRoleArn = self.customRoleArnInternal {
            keychain.setString(customRoleArn, forKey: CustomRoleArnKey)
        } else {
            keychain.removeItem(forKey: CustomRoleArnKey)
        }
        if federationDisabled {
            keychain.setString("true", forKey: FederationDisabledKey)
        }
    }

    internal func saveOAuthURIQueryParametersInKeychain() {
        keychain.setData(JSONHelper.dataFromDictionary(signInURIQueryParameters), forKey: SignInURIQueryParametersKey)
        keychain.setData(JSONHelper.dataFromDictionary(tokenURIQueryParameters), forKey: TokenURIQueryParametersKey)
        keychain.setData(JSONHelper.dataFromDictionary(signOutURIQueryParameters), forKey: SignOutURIQueryParametersKey)
    }

    internal func loadOAuthURIQueryParametersFromKeychain() {
        signInURIQueryParameters = JSONHelper.dictionaryFromData(keychain.data(forKey: SignInURIQueryParametersKey))
        tokenURIQueryParameters = JSONHelper.dictionaryFromData(keychain.data(forKey: TokenURIQueryParametersKey))
        signOutURIQueryParameters = JSONHelper.dictionaryFromData(keychain.data(forKey: SignOutURIQueryParametersKey))
    }

    internal func saveLoginsMapInKeychain() {
        if cachedLoginsMap.count == 0 {
            keychain.removeItem(forKey: LoginsMapKey)
            keychain.removeItem(forKey: FederationDisabledKey)
        } else {
            do {
                let data = try Data(base64Encoded: JSONEncoder().encode(cachedLoginsMap).base64EncodedData())
                keychain.setData(data, forKey: LoginsMapKey)
            } catch {
    ...
    internal func loadLoginsMapFromKeychain() {
        let data = keychain.data(forKey: LoginsMapKey)
        if data != nil {
            do {
                let dict = try JSONDecoder().decode([String: String].self, from: data!)
                cachedLoginsMap = dict
            } catch {
    ...
    internal func loadFederationProviderMetadataFromKeychain() {
        ...
        if let customRoleArnString = self.keychain.string(forKey: CustomRoleArnKey) {
            customRoleArnInternal = customRoleArnString
        }
