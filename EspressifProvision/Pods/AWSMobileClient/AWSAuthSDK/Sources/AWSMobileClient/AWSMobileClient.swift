...
public final class AWSMobileClient: _AWSMobileClient {
    ...
    var keychain: AWSUICKeyChainStore = AWSUICKeyChainStore(service: "\(String(describing: Bundle.main.bundleIdentifier)).AWSMobileClient")
    ...
    public var username: String? {
        return userpoolOpsHelper.currentActiveUser?.username
    }
    ...
    public override var identityId: String? {
        return internalCredentialsProvider?.identityId
    }
    ...
    @objc public var isSignedIn: Bool {
        if operateInLegacyMode {
            ...
            return cachedLoginsMap.count > 0
        }
    }
    ...
    public func handleAuthResponse(_ application: UIApplication, open url: URL, sourceApplication _: String?, annotation _: Any) {
        if isCognitoAuthRegistered {
            AWSCognitoAuth(forKey: CognitoAuthRegistrationKey).application(application, open: url, options: [:])
        }
    }
    ...
    public func initialize(_ completionHandler: @escaping (UserState?, Error?) -> Void) {
        ...
        initializationQueue.sync {
            ...
            if self.federationProvider == .hostedUI {
                ...
                let cognitoAuthConfig: AWSCognitoAuthConfiguration = AWSCognitoAuthConfiguration(appClientId: clientId!,
                ...
                let cognitoAuth = AWSCognitoAuth(forKey: CognitoAuthRegistrationKey)
    ...
    public func removeUserStateListener(_ object: AnyObject) {
        listeners = listeners.filter { !($0.0 === object) }
    }
    ...
    public func showSignIn(navigationController: UINavigationController,
        ...
        if let hostedUIOptions = hostedUIOptions {
            ...
            let signInRedirectURI = infoDictionary?["SignInRedirectURI"] as? String
            let signInURI = infoDictionary?["SignInURI"] as? String
            if signInURIQueryParameters == nil {
                signInURIQueryParameters = infoDictionary?["SignInURIQueryParameters"] as? [String: String]
            }

            let signOutRedirectURI = infoDictionary?["SignOutRedirectURI"] as? String
            let signOutURI = infoDictionary?["SignOutURI"] as? String
            if signOutURIQueryParameters == nil {
                signOutURIQueryParameters = infoDictionary?["SignOutURIQueryParameters"] as? [String: String]
            }

            let tokensURI = infoDictionary?["TokenURI"] as? String
            if tokenURIQueryParameters == nil {
                tokenURIQueryParameters = infoDictionary?["TokenURIQueryParameters"] as? [String: String]
            }
            ...
            if hostedUIOptions.signInURIQueryParameters != nil {
                signInURIQueryParameters = hostedUIOptions.signInURIQueryParameters
            }

            if hostedUIOptions.tokenURIQueryParameters != nil {
                tokenURIQueryParameters = hostedUIOptions.tokenURIQueryParameters
            }

            if hostedUIOptions.signOutURIQueryParameters != nil {
                signOutURIQueryParameters = hostedUIOptions.signOutURIQueryParameters
            }
            ...
            showSign(inScreen: navigationController, signInUIConfiguration: signInUIOptions, completionHandler: { providerName, token, error in
                if error == nil {
...
extension AWSMobileClient {
    ...
    public override func invalidateCachedTemporaryCredentials() {
        internalCredentialsProvider?.invalidateCachedTemporaryCredentials()
    }
    ...
    public override func getIdentityId() -> AWSTask<NSString> {
        guard internalCredentialsProvider != nil else {
        ...
        internalCredentialsProvider?.getIdentityId().continueWith(block: { (task) -> Any? in
