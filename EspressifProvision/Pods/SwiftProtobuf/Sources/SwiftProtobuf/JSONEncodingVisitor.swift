...
internal struct JSONEncodingVisitor: Visitor {
    ...
    init(type: Message.Type, options: JSONEncodingOptions) throws {
        if let nameProviding = type as? _ProtoNameProviding.Type {
            nameMap = nameProviding._protobuf_nameMap
        } else {
    ...
    mutating func visitSingularGroupField<G: Message>(value _: G, fieldNumber _: Int) throws {
    ...
    mutating func visitRepeatedGroupField<G: Message>(value _: [G], fieldNumber _: Int) throws {
        // Google does not serialize groups into JSON
    }
    ...
    mutating func visitMapField<KeyType, ValueType: MapValueType>(fieldType _: _ProtobufMap<KeyType, ValueType>.Type, value: _ProtobufMap<KeyType, ValueType>.BaseType, fieldNumber: Int) throws {
        try startField(for: fieldNumber)
        encoder.append(text: "{")
    ...
    mutating func visitMapField<KeyType, ValueType>(fieldType _: _ProtobufEnumMap<KeyType, ValueType>.Type, value: _ProtobufEnumMap<KeyType, ValueType>.BaseType, fieldNumber: Int) throws where ValueType.RawValue == Int {
        try startField(for: fieldNumber)
        encoder.append(text: "{")
        var mapVisitor = JSONMapEncodingVisitor(encoder: encoder, options: options)
        for (k, v) in value {
    ...
    mutating func visitMapField<KeyType, ValueType>(fieldType _: _ProtobufMessageMap<KeyType, ValueType>.Type, value: _ProtobufMessageMap<KeyType, ValueType>.BaseType, fieldNumber: Int) throws {
        try startField(for: fieldNumber)
        encoder.append(text: "{")
        var mapVisitor = JSONMapEncodingVisitor(encoder: encoder, options: options)
        for (k, v) in value {
    ...
    mutating func visitExtensionFields(fields _: ExtensionFieldValueSet, start _: Int, end _: Int) throws {
