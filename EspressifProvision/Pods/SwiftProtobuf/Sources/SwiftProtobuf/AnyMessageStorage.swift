...
internal class AnyMessageStorage {
    ...
    var _value: Data {
        ...
        get {
            ...
            case let .contentJSON(contentJSON, options):
    ...
    func isA<M: Message>(_: M.Type) -> Bool {
        if _typeURL.isEmpty {
            return false
        }
...
extension AnyMessageStorage {
    ...
    func isEqualTo(other: AnyMessageStorage) -> Bool {
        ...
        if case let .message(myMsg) = state, case let .message(otherMsg) = other.state, type(of: myMsg) == type(of: otherMsg) {
        ...
        if case let .binary(myValue) = state, case let .binary(otherValue) = other.state, myValue == otherValue {
            return true
        }
